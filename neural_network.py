...
#! It works, but I think maybe use more data on training.
...
# use 5 fold corss validation on the data
cross_validation_scores = cross_val_score(model, X_train, y_train, cv=5)

# pytorch way
num_folds = 5
# get random split from dataset
kf = StratifiedKFold(n_splits=num_folds, shuffle=True, random_state=42)

# Perform cross-validation
cv_scores = []
#? I guess bow_arry and labels are panda objects?
X = bow_array
y = labels
...
for train_index, val_index in kf.split(X, y):
    ...
    with torch.no_grad():
        ...
        print(f'Validation Loss: {vall_loss.item()}')
        accuracy = accuracy_score(y_val_tensor.numpy(), y_pred.argmax(dim=1).numpy())
        cv_scores.append(accuracy)
    # didn't include the last part because not sure how it works
    

print("Cross-validation scores:", cv_scores)
print("Mean CV score:", sum(cv_scores) / num_folds)
